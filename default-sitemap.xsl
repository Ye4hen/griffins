...
>
	...
	<a href="https://ye4hen.github.io/griffins"><span>Головна</span></a>
	...
	<a href="https://ye4hen.github.io/griffins"><span>Головна</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="https://ye4hen.github.io/griffins/sitemap.xml"><span>Sitemap Index</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<p>Згенеровано <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, це XML-карта сайту, призначена для використання пошуковими системами, такими як Google або Bing.</p>
		<p>
			Ви можете дізнатись більше про XML-карти сайту на <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Ця карта сайту містить						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								URL							</xsl:when>
							<xsl:otherwise>
								Посилання							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						Індекс цієї карти сайту містить						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								sitemap							</xsl:when>
							<xsl:otherwise>
								карти сайту							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				and was generated on 17.09.2023 at 06:54			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			...
			<xsl:when test="$priorityLevel = 'medium'">Середній</xsl:when>
	...
	<a href="https://ye4hen.github.io/griffins"><span>Головна</span></a>
		...
		<div class="empty-sitemap__buttons">
			<a href="https://ye4hen.github.io/griffins" class="button">Back to Homepage</a>
					</div>
